...
bool merge(vector<ftype>& Y, vector<ftype>& X, vector<ftype>& errorX, vector<ftype>& clusterProbabilities, vector<vector<ftype>>& associationMatrix, ftype& T, const int& N) {
	for (int j = 0; (j + 1) < Y.size(); ++j) {
		for (int k = j + 1; k < Y.size(); ++k) {
			...
			if (fabs(Y[j] - Y[k]) < 0.001) { // Need to merge
...
pair<vector<int>, vector<ftype>> runDA(event_t event, int nSWEEPS) {
	...
	ftype beta = 1.0 / T;
    ftype coolingFactor = exp(-(log(T / Tmin) / nSweeps));
	...
	while (T > Tmin) {
		...
		if (useLinearCooling) {
			...
			beta = beta / coolingFactor; // Geometric cooling in beta
            T = 1.0 / beta;
		}
	...
	unsigned int ntry = 0;
	while (split(X, errorX, clusterCentroids, associationMatrix, clusterProbabilities, T, N, delta) && ntry++ < 10 && clusterCentroids.size() < Kmax) {
