...
declare -a sweeps=(25 27) #2 5 10 20)
...
make -C ${PROJECT_DIR} -f \${makefile} && make -C ${PROJECT_DIR} -f \${makefile} run THREADS=${THREADS} STAGES=${STAGES} SWEEPS=${SWEEPS} DA_SWEEPS=${SWEEPS} SAMPLES_PER_THREAD=${SAMPLES_PER_THREAD}
EOF
